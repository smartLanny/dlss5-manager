...
function preferred(g, slot) {
  if (g.ab?.[slot]) return getPkg(g.ab[slot]);
  if (slot === 'B') return [...state.packages].reverse().find(p => /beta/i.test(p.manifest.version)) || state.packages.at(-1);
  return state.packages.find(p => p.manifest.version === '0.3.3.4') || state.packages.find(p => !/beta|local/i.test(p.manifest.version)) || state.packages.at(-1);
}
...
function renderHistory() {
  ...
  $('historyList').innerHTML = state.history.length ? state.history.map(h => `<article class="history-row"><span class="symbol">↶</span><div class="history-info"><h3>${esc(getGame(h.gameId)?.name || '游戏')} · ${h.operation === 'uninstall' ? '恢复原版' : esc(version(h.version))}</h3><p>${esc(when(h.createdAt))} · ${pending.has(h.status) ? '需要恢复' : h.status === 'reverted' ? '已恢复' : h.status === 'aborted' ? '未改动游戏' : '操作完成'}</p></div>${latest.get(h.gameId) === h.id ? `<button class="button secondary" data-recover="${esc(h.id)}" data-game-id="${esc(h.gameId)}">恢复这次操作前</button>` : '<span class="muted">记录保留</span>'}</article>`).join('') : '<div class="empty-state"><div class="symbol">↶</div><h3>还没有安装记录</h3><p>安装或换版本后，可以在这里恢复。</p></div>';
}
...
async function gameAction(action, id) {
  ...
  return run(action === 'install' ? '正在准备安装' : '正在处理', async () => {
    ...
    if (action === 'uninstall') { if (await api('uninstall', { gameId:id })) toast('已恢复原版，游戏设置和其他模组仍保留。'); await refresh(); }
  });
}
