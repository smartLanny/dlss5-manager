...
async function main() {
  ...
  for (let i = 0; i < args.length; i += 2) {
    if (!['--addon', '--runtime', '--version', '--api', '--out', '--sign-key', '--key-id'].includes(args[i]) || !args[i + 1]) throw new Error('参数不完整或未知。');
  ...
  for (const [input, role] of [[opts.addon, 'addon'], [opts.runtime, 'nr-runtime']]) {
    ...
    entries.push([name, data]); files.push({ path: name, role, sha256: sha256(data) });
  }
  const manifest = validateManifest({ schema: 1, component: 'nr-before-sr', architecture: 'x64', version: opts.version, apis: opts.api.split(','), files,
