...
async function main() {
  ...
  try {
    ...
    if (process.platform === 'win32') {
      ...
      const beforeRepeat = await page.evaluate(async () => (await window.manager.state()).value.history.length);
      await page.locator('[data-action="preview"]').click(); await page.waitForFunction(() => document.querySelector('#planDialog').open && document.body.getAttribute('aria-busy') !== 'true');
      assert.ok((await page.locator('#planContent').innerText()).includes('仅校验'));
      await page.locator('#confirmCompatibility').check(); await page.locator('#confirmPlan').check(); await page.locator('#confirmApply').click();
      await page.waitForFunction(() => !document.querySelector('#planDialog').open && document.body.getAttribute('aria-busy') !== 'true');
      assert.equal(await page.evaluate(async () => (await window.manager.state()).value.history.length), beforeRepeat);
    ...
    const feedbackFile = path.join(root, 'synthetic-feedback.zip'), privateLog = Buffer.from('Synthetic private path C:\\Users\\PrivateTester\\runtime.log');
    const feedbackManifest = { schemaVersion: 1, producerId: 'nr-before-sr', producerVersion: '0.4.2beta', createdAt: new Date().toISOString(), gameExeName: 'DemoGame.exe', api: 'DX12', files: [{ path: 'runtime.log', role: 'log', sha256: sha256(privateLog), sensitivity: 'private' }] };
    await fs.writeFile(feedbackFile, writeZip([['feedback-manifest.json', Buffer.from(JSON.stringify(feedbackManifest))], ['runtime.log', privateLog]]));
    await page.locator('.nav-item[data-page="feedback"]').click();
    await page.locator('#feedbackType').selectOption('image');
    await page.locator('#feedbackSymptom').fill('合成测试：出现闪烁\nC:\\Users\\PrivateTester\\log.txt\nmail@example.org');
    await page.locator('#feedbackSteps').fill('打开合成测试游戏，切换场景。');
    await application.evaluate(({ dialog }, filePaths) => { dialog.showOpenDialog = async () => ({ canceled: false, filePaths }); }, [feedbackFile]);
    await page.locator('#feedbackImport').click(); await page.waitForFunction(() => document.body.getAttribute('aria-busy') !== 'true');
    assert.ok((await page.locator('#feedbackAttachment').innerText()).includes('清单与哈希一致'));
    await page.locator('#feedbackGenerate').click(); await page.waitForFunction(() => !document.querySelector('#feedbackReviewed').disabled && document.body.getAttribute('aria-busy') !== 'true');
    const publicText = await page.locator('#feedbackReport').innerText();
    for (const value of ['PrivateTester', 'mail@example.org', 'DemoGame.exe', 'Synthetic private path']) assert.ok(!publicText.includes(value), value);
    assert.equal(await page.locator('#feedbackCopy').isDisabled(), true);
    await page.locator('#feedbackReviewed').check(); await page.locator('#feedbackCopy').click(); await page.waitForFunction(() => document.body.getAttribute('aria-busy') !== 'true');
    assert.equal(await application.evaluate(() => globalThis.__smokeClipboard), publicText);
    await page.locator('#feedbackOpen').click(); await page.waitForFunction(() => document.body.getAttribute('aria-busy') !== 'true');
    const openedIssue = await application.evaluate(() => globalThis.__smokeOpened);
    assert.ok(openedIssue.startsWith('https://github.com/smartLanny/dlss5-manager/issues/new?title=')); assert.ok(!openedIssue.includes('body='));
    const publicFile = path.join(root, 'public-report.md');
    await application.evaluate(({ dialog }, filePath) => { dialog.showSaveDialog = async () => ({ canceled: false, filePath }); }, publicFile);
    await page.locator('#feedbackPublic').click(); await page.waitForFunction(() => document.body.getAttribute('aria-busy') !== 'true');
    assert.equal(await fs.readFile(publicFile, 'utf8'), publicText);
    const privateFile = path.join(root, 'diagnostic-private.zip');
    await application.evaluate(({ dialog }, filePath) => { dialog.showSaveDialog = async () => ({ canceled: false, filePath }); }, privateFile);
    await page.locator('#feedbackPrivate').click(); await page.waitForFunction(() => document.body.getAttribute('aria-busy') !== 'true');
    assert.equal(readZip(await fs.readFile(privateFile)).has('runtime-feedback.zip'), false);
    await page.locator('#feedbackIncludeAttachment').check(); await page.locator('#feedbackPrivate').click(); await page.waitForFunction(() => document.body.getAttribute('aria-busy') !== 'true');
    assert.deepEqual(readZip(await fs.readFile(privateFile)).get('runtime-feedback.zip').bytes, await fs.readFile(feedbackFile));
    await page.locator('#feedbackSymptom').fill('合成测试示例：切换场景后出现闪烁。');
    assert.equal(await page.locator('#feedbackCopy').isDisabled(), true);
    await page.locator('#feedbackGenerate').click(); await page.waitForFunction(() => !document.querySelector('#feedbackReviewed').disabled && document.body.getAttribute('aria-busy') !== 'true');
    await page.screenshot({ path: 'output/ui-feedback.png' });
    assert.deepEqual(errors, []);
    console.log('Electron smoke passed: real local UI, sandbox/IPC isolation, three version entries, SHA-256 import, game configuration, ' + (process.platform === 'win32' ? 'real Windows file installation and rollback' : 'non-Windows write gate') + ', exact Bilibili URL, private diagnostics, feedback manifest import, public redaction, review consent, no automatic issue creation, explicit private attachment export.');
  } finally {
