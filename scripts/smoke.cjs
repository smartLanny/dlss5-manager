...
const {pe}=require('../test/helpers.cjs');const {sha256,digestFile}=require('../src/core/safety.cjs');const {writeZip}=require('../src/core/packages.cjs');const {PIN,ensureReShade}=require('../src/core/reshade.cjs');
...
async function main(){
 ...
 const exe=path.join(gameRoot,'DemoGame.exe'),aFile=path.join(root,'0.3.3.4.dlss5pkg'),bFile=path.join(root,'0.4.2beta.dlss5pkg');
 const exeData=pe(0,false);Buffer.from('D3D12CreateDevice').copy(exeData,550);await fs.writeFile(exe,exeData);await fs.writeFile(aFile,bundle('0.3.3.4',1));await fs.writeFile(bFile,bundle('0.4.2beta',2));
 ...
 try{
  ...
  s=await page.evaluate(async()=>(await window.manager.state()).value);assert.equal(s.packages.length,2);const a=s.packages.find(p=>p.manifest.version==='0.3.3.4'),b=s.packages.find(p=>p.manifest.version==='0.4.2beta');
  ...
  await page.locator('.nav[data-page="feedback"]').click();await page.locator('#feedbackType').selectOption('install');await page.locator('#feedbackSymptom').fill('测试反馈：切换版本后需要确认安装记录。');await page.locator('#feedbackForm button[type="submit"]').click();await idle();
  assert.ok((await page.locator('#feedbackReport').innerText()).includes('0.2.0-beta.2'));await page.screenshot({path:'output/ui-feedback.png'});
  ...
  console.log('PLAYER UI PASSED: no API/hash form, automatic EXE/API, multi-version import, real pinned ReShade plus synthetic addon/runtime transaction, A/B/A, repeated install no-op, original settings preserved, rollback removes owned loader, feedback and Bilibili. All game files synthetic; no game executable was launched.');
